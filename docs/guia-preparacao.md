...
chat completions — inclusive uma faixa de modelos gratuitos, que é a que a
aula usa.
...
**O que observar:** uma chave no formato `sk-or-...`. Não é preciso
...
a sua chave (e não vai para o git — está no `.gitignore`).
...
requisição ele seleciona automaticamente um dos ~24 modelos de variante
gratuita da plataforma, com suporte a tool calling e contexto de 200k tokens
— sem cobrança de tokens. É a forma mais simples de inferência gratuita, e é
...
modelo atendeu — com `openrouter/free`, pode variar entre execuções (bom
lembrete de que o contrato é o mesmo para todos).
...
via Ollama) — é a portabilidade do padrão chat completions que o degrau 02
ensina. Aviso: modelos de 7B obedecem pior às ferramentas — os agentes da
aula vão precisar de mais passos.
