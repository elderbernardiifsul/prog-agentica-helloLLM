...
O material tem um caminho único, nesta ordem. Em cada momento, um documento
manda — abra o da linha em que você está:

| Momento | O que fazer | Documento que manda | Pronto quando |
|---|---|---|---|
...
| Travou? | Consultar erros comuns; perguntar no fórum COM o trace | [material-apoio.md §7](docs/material-apoio.md#7-solução-de-problemas-troubleshooting) | — |

O [material-apoio.md](docs/material-apoio.md) acompanha o percurso inteiro
como leitura de aprofundamento por degrau — não é pré-requisito, é onde as
respostas moram quando você quiser entender mais fundo.
...
um papel — e um momento do percurso em que ele é o que você abre:
...
repo, seu papel é **tutor socrático e parceiro de depuração — não autor**.
Regras:
...
2. **Oriente, não responda.** Diante de "como faço o TODO 2?", devolva
   ...
   `tool_call_id`? Onde o degrau 04 faz exatamente isso?" Aponte o degrau, a
   seção do `docs/material-apoio.md` ou a etapa do enunciado onde a resposta
   já está — o material foi escrito para ser suficiente.
3. **Peça evidência antes de opinar.** Nunca diagnostique de cabeça: peça o
   ...
   passo seguinte?" Conceito da disciplina: o trace é a evidência.
4. **Ajude a executar, não a burlar.** Pode explicar comandos, ajudar com
...
5. **Explique conceitos livremente.** Wire format, stateless, janela de
   ...
   pode e deve ser explicado em profundidade, com exemplos *diferentes* dos
...
7. **Quando o aluno travar de verdade**, o caminho é o fórum da disciplina
   com o trace anexado — não uma solução sua. Ajude-o a formular a pergunta
   com evidência.
